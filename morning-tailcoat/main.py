"""
Morning Coat / Cutaway — Fashion Cabinet Garment Cartridge
(FC-400 #314, tailoring, T4).

Formal daywear, the descendant of the riding coat: below the single fastening
button the centre-front edge SWEEPS AWAY in a continuous cutaway curve to the side
seam, and the coat continues behind as long tails. The cutaway curve IS the
garment — it is not a hem variant, it changes which edges exist and where the
button sits relative to the sweep.

Three things are solved by measurement rather than by formula:

  1. THE CUTAWAY CURVE IS SOLVED OVER A MEASURED SPAN, NOT SKETCHED. The sweep
     runs from the MEASURED fastening point down and back to the side seam at the
     MEASURED tail break. It is drawn as a Bezier whose span (horizontal run and
     vertical drop) is derived from the button height and the side, and BOTH the
     rise and the run are floored — at parameter extremes the derived span goes to
     zero or NEGATIVE, and a negative span does not fail, it inverts the front into
     geometry the kernel's CCW normalization launders into a valid-LOOKING outline.

  2. THE TAILS ARE A SEPARATE SKIRT ON THEIR OWN GRAIN, WAIST-SEAMED. The back
     below the waist is a separate TAIL piece, not a continuation of the body, so
     it can be cut on its own grain and hang. Its top is drafted to the MEASURED
     back waist so the waist seam closes.

  3. THE BUTTON, THE LAPEL FACING AND THE SLEEVE ARE SOLVED OFF MEASUREMENTS. The
     single button sits at the measured waist break; the peak lapel facing is
     derived from the measured lapel; the two-piece sleeve's vertical seams close
     by construction and its cap is eased to the measured armscye.

TAILORING CONVENTIONS: edge-stitch; a waist-seamed tail; a two-piece sleeve. The
SEW-THROUGH BUTTON SOLID is Yantra4D territory (`sew-through-button`; see
notion.hardware_ref).

Sandbox contract (apps/api/services/engine/fc_runner.py): `fc`/`math` pre-injected;
params as bare globals via PARAM(lambda...); result = a top-level fc.PatternSet.
"""

import math

import fc


def PARAM(getter, default):
    try:
        v = getter()
        return default if v is None else v
    except Exception:
        return default


target_piece = str(PARAM(lambda: target_piece, "set"))
# front|back|tail|upper_sleeve|under_sleeve|collar|set

chest_girth = float(PARAM(lambda: chest_girth, 1020.0))
waist_girth = float(PARAM(lambda: waist_girth, 880.0))
back_width = float(PARAM(lambda: back_width, 460.0))
back_length = float(PARAM(lambda: back_length, 440.0))
tail_length = float(PARAM(lambda: tail_length, 400.0))       # waist to tail hem
sleeve_length = float(PARAM(lambda: sleeve_length, 630.0))
neck_width = float(PARAM(lambda: neck_width, 170.0))
button_rise = float(PARAM(lambda: button_rise, 40.0))        # button above the waist
cutaway_sweep = float(PARAM(lambda: cutaway_sweep, 220.0))   # horizontal run of the sweep
button_ligne = float(PARAM(lambda: button_ligne, 34.0))      # fastening button size
lapel_width = float(PARAM(lambda: lapel_width, 95.0))
coat_ease = float(PARAM(lambda: coat_ease, 110.0))
seam_allowance = float(PARAM(lambda: seam_allowance, 12.0))
hem_allowance = float(PARAM(lambda: hem_allowance, 40.0))

chest_girth = max(820.0, min(chest_girth, 1400.0))
waist_girth = max(640.0, min(waist_girth, 1300.0))
back_width = max(360.0, min(back_width, 540.0))
back_length = max(360.0, min(back_length, 520.0))
tail_length = max(280.0, min(tail_length, 600.0))
sleeve_length = max(520.0, min(sleeve_length, 720.0))
neck_width = max(140.0, min(neck_width, 220.0))
button_rise = max(0.0, min(button_rise, 120.0))
cutaway_sweep = max(80.0, min(cutaway_sweep, 400.0))
button_ligne = max(28.0, min(button_ligne, 46.0))
lapel_width = max(60.0, min(lapel_width, 140.0))
coat_ease = max(60.0, min(coat_ease, 220.0))
seam_allowance = max(8.0, min(seam_allowance, 18.0))
hem_allowance = max(28.0, min(hem_allowance, 60.0))

EDGESTITCH = 6.0
BUTTON_MM = button_ligne * 0.635

QUARTER_CHEST = (chest_girth + coat_ease) / 4.0
QUARTER_WAIST = max(QUARTER_CHEST * 0.70,
                    min((waist_girth + coat_ease) / 4.0, QUARTER_CHEST - 6.0))
HALF_NECK = neck_width / 2.0
HALF_BACK = back_width / 2.0
NECK_DROP_B = max(20.0, HALF_NECK * 0.28)
SHOULDER_SLOPE = 45.0
BODICE_LEN = back_length

# The cutaway span: horizontal run and vertical drop, both floored so the sweep
# can never invert. The button break is above the waist by button_rise.
BUTTON_Y = BODICE_LEN + button_rise
_SWEEP_RUN_RAW = cutaway_sweep
SWEEP_RUN = max(40.0, min(_SWEEP_RUN_RAW, QUARTER_WAIST + 40.0))
_SWEEP_DROP_RAW = BUTTON_Y - 0.0            # from the break down to the waist front
SWEEP_DROP = max(30.0, _SWEEP_DROP_RAW)


def _button(label, x, y):
    r = BUTTON_MM / 2.0
    ring = [fc.P(x + r * math.cos(math.radians(a)),
                 y + r * math.sin(math.radians(a))) for a in range(0, 361, 30)]
    return fc.Internal(label, ring + [fc.P(x, y)], kind="drill")


def build_front():
    """Front to the waist, cut 2 mirrored. The cutaway sweeps from the button to
    the side."""
    hw = QUARTER_CHEST
    # The break/button is at CF at BUTTON_Y; the front edge sweeps from there back
    # to the side at the waist.
    p_break = fc.P(0.0, BUTTON_Y)
    p_side_waist = fc.P(SWEEP_RUN, 0.0)         # where the sweep meets the waist edge
    p_waist_side = fc.P(QUARTER_WAIST, 0.0)
    p_armhole = fc.P(hw, BODICE_LEN - QUARTER_CHEST * 0.5)
    p_shoulder_pt = fc.P(HALF_BACK, BODICE_LEN - SHOULDER_SLOPE)
    p_neck_pt = fc.P(HALF_NECK, BODICE_LEN)
    p_lapel_pt = fc.P(-lapel_width * 0.25, BUTTON_Y + lapel_width * 0.7)
    edges = [
        # The short waist edge under the sweep (from the sweep's end to the side).
        fc.Edge("waist", [fc.Line(p_side_waist, p_waist_side)]),
        fc.Edge("side", [fc.Line(p_waist_side, p_armhole)]),
        fc.Edge("armscye", [fc.Bezier(
            p_armhole, fc.P(hw - 10.0, BODICE_LEN - QUARTER_CHEST * 0.24),
            fc.P(HALF_BACK + 12.0, BODICE_LEN - SHOULDER_SLOPE + 20.0), p_shoulder_pt)]),
        fc.Edge("shoulder", [fc.Line(p_shoulder_pt, p_neck_pt)]),
        fc.Edge("gorge", [fc.Line(p_neck_pt, p_lapel_pt)]),
        fc.Edge("lapel", [fc.Line(p_lapel_pt, p_break)]),
        # THE CUTAWAY SWEEP: a Bezier from the break down and back to the side, its
        # span solved from SWEEP_RUN and SWEEP_DROP (both floored).
        fc.Edge("cutaway", [fc.Bezier(
            p_break,
            fc.P(SWEEP_RUN * 0.12, BUTTON_Y - SWEEP_DROP * 0.25),
            fc.P(SWEEP_RUN * 0.55, SWEEP_DROP * 0.30),
            p_side_waist)]),
    ]
    internals = [
        fc.Internal("roll line", [fc.P(p_neck_pt.x, p_neck_pt.y), fc.P(0.0, BUTTON_Y)],
                    kind="marking"),
        _button("fastening button", BUTTON_MM * 0.7, BUTTON_Y),
    ]
    return fc.Piece(
        "front", edges,
        seam_allowance=seam_allowance,
        allowances={},
        notches=[fc.Notch("armscye", 0.0, "underarm"),
                 fc.Notch("waist", 1.0, "side seam"),
                 fc.Notch("cutaway", 0.0, "button break")],
        grainline=fc.Grainline(fc.P(hw * 0.4, 20.0), fc.P(hw * 0.4, BODICE_LEN - 20.0)),
        internals=internals,
        cut=fc.CutSpec(quantity=2, mirror=True),
        label="Front (cut 2, mirrored)",
    )


def build_back():
    hw = QUARTER_CHEST
    p_waist_cb = fc.P(0.0, 0.0)
    p_waist_side = fc.P(QUARTER_WAIST, 0.0)
    p_armhole = fc.P(hw, BODICE_LEN - QUARTER_CHEST * 0.5)
    p_shoulder_pt = fc.P(HALF_BACK, BODICE_LEN - SHOULDER_SLOPE)
    p_neck_pt = fc.P(HALF_NECK, BODICE_LEN)
    p_neck_cb = fc.P(0.0, BODICE_LEN - NECK_DROP_B)
    edges = [
        fc.Edge("waist", [fc.Line(p_waist_cb, p_waist_side)]),
        fc.Edge("side", [fc.Line(p_waist_side, p_armhole)]),
        fc.Edge("armscye", [fc.Bezier(
            p_armhole, fc.P(hw - 10.0, BODICE_LEN - QUARTER_CHEST * 0.24),
            fc.P(HALF_BACK + 12.0, BODICE_LEN - SHOULDER_SLOPE + 20.0), p_shoulder_pt)]),
        fc.Edge("shoulder", [fc.Line(p_shoulder_pt, p_neck_pt)]),
        fc.Edge("neck", [fc.curve_through(p_neck_pt, p_neck_cb, bulge=0.16, side=1.0)]),
        fc.Edge("cb", [fc.Line(p_neck_cb, p_waist_cb)]),
    ]
    return fc.Piece(
        "back", edges,
        seam_allowance=seam_allowance,
        allowances={},
        notches=[fc.Notch("waist", 1.0, "side seam / tail match"),
                 fc.Notch("armscye", 0.0, "underarm")],
        grainline=fc.Grainline(fc.P(hw * 0.35, 20.0), fc.P(hw * 0.35, BODICE_LEN - 20.0)),
        internals=[],
        cut=fc.CutSpec(quantity=2, mirror=True),
        label="Back (cut 2, mirrored)",
    )


def build_tail():
    """The tail skirt, cut 2 mirrored. Waist-seamed to the back on its own grain."""
    p_hem_cb = fc.P(0.0, 0.0)
    p_hem_side = fc.P(QUARTER_WAIST, 0.0)
    p_waist_side = fc.P(QUARTER_WAIST, tail_length)
    p_waist_cb = fc.P(0.0, tail_length)
    edges = [
        fc.Edge("hem", [fc.Line(p_hem_cb, p_hem_side)]),
        fc.Edge("side", [fc.Line(p_hem_side, p_waist_side)]),
        fc.Edge("waist", [fc.Line(p_waist_side, p_waist_cb)]),
        fc.Edge("cb_vent", [fc.Line(p_waist_cb, p_hem_cb)]),
    ]
    return fc.Piece(
        "tail", edges,
        seam_allowance=seam_allowance,
        allowances={"hem": hem_allowance},
        notches=[fc.Notch("waist", 1.0, "back waist match"),
                 fc.Notch("side", 1.0, "side seam")],
        grainline=fc.Grainline(fc.P(QUARTER_WAIST * 0.4, 20.0),
                               fc.P(QUARTER_WAIST * 0.4, tail_length - 20.0)),
        internals=[
            fc.Internal("CB vent", [fc.P(0.0, 0.0), fc.P(0.0, tail_length * 0.6)],
                        kind="marking"),
            fc.Internal("waist button (tail)", [fc.P(QUARTER_WAIST * 0.5, tail_length - 20.0)],
                        kind="drill"),
        ],
        cut=fc.CutSpec(quantity=2, mirror=True),
        label="Tail (cut 2, mirrored)",
    )


_FRONT = build_front()
_BACK = build_back()
ARMSCYE_RUN = _FRONT.edge("armscye").length(0.05) + _BACK.edge("armscye").length(0.05)
LAPEL_RUN = _FRONT.edge("lapel").length(0.05) + _FRONT.edge("gorge").length(0.05)
CUTAWAY_RUN = _FRONT.edge("cutaway").length(0.05)


def build_upper_sleeve():
    sw = QUARTER_CHEST * 0.86
    cap_h = QUARTER_CHEST * 0.32
    ln = sleeve_length
    edges = [
        fc.Edge("cap", [fc.Bezier(
            fc.P(sw, 0.0), fc.P(sw * 0.80, cap_h * 0.9),
            fc.P(sw * 0.30, cap_h * 1.05), fc.P(0.0, 0.0))]),
        fc.Edge("fore_seam", [fc.Line(fc.P(0.0, 0.0), fc.P(0.0, -ln))]),
        fc.Edge("cuff", [fc.Line(fc.P(0.0, -ln), fc.P(sw, -ln))]),
        fc.Edge("hind_seam", [fc.Line(fc.P(sw, -ln), fc.P(sw, 0.0))]),
    ]
    return fc.Piece(
        "upper_sleeve", edges,
        seam_allowance=seam_allowance,
        allowances={"cuff": hem_allowance},
        notches=[fc.Notch("cap", 0.5, "shoulder point")],
        grainline=fc.Grainline(fc.P(sw / 2.0, -ln * 0.1), fc.P(sw / 2.0, cap_h * 0.9)),
        internals=[],
        cut=fc.CutSpec(quantity=2, mirror=True),
        label="Upper sleeve (cut 2, mirrored)",
    )


def build_under_sleeve():
    sw = QUARTER_CHEST * 0.40
    ln = sleeve_length
    edges = [
        fc.Edge("scye", [fc.curve_through(
            fc.P(0.0, 0.0), fc.P(sw, 0.0), bulge=0.20, side=1.0)]),
        fc.Edge("hind_seam", [fc.Line(fc.P(sw, 0.0), fc.P(sw, -ln))]),
        fc.Edge("cuff", [fc.Line(fc.P(sw, -ln), fc.P(0.0, -ln))]),
        fc.Edge("fore_seam", [fc.Line(fc.P(0.0, -ln), fc.P(0.0, 0.0))]),
    ]
    return fc.Piece(
        "under_sleeve", edges,
        seam_allowance=seam_allowance,
        allowances={"cuff": hem_allowance},
        notches=[fc.Notch("scye", 0.5, "underarm")],
        grainline=fc.Grainline(fc.P(sw / 2.0, -ln * 0.1), fc.P(sw / 2.0, -ln * 0.85)),
        internals=[],
        cut=fc.CutSpec(quantity=2, mirror=True),
        label="Under sleeve (cut 2, mirrored)",
    )


def build_collar():
    gorge_run = _FRONT.edge("gorge").length(0.05) * 2.0
    back_neck = _BACK.edge("neck").length(0.05) * 2.0
    ln = (gorge_run + back_neck) / 2.0
    depth = max(45.0, HALF_NECK * 0.5)
    edges = [
        fc.Edge("cb_fold", [fc.Line(fc.P(0.0, 0.0), fc.P(0.0, depth))]),
        fc.Edge("fall", [fc.Line(fc.P(0.0, depth), fc.P(ln, depth))]),
        fc.Edge("cf_end", [fc.Line(fc.P(ln, depth), fc.P(ln, 0.0))]),
        fc.Edge("neck_edge", [fc.curve_through(
            fc.P(ln, 0.0), fc.P(0.0, 0.0), bulge=0.12, side=1.0)]),
    ]
    return fc.Piece(
        "collar", edges,
        seam_allowance=seam_allowance,
        allowances={"cb_fold": 0.0},
        notches=[fc.Notch("neck_edge", 0.5, "shoulder match")],
        grainline=fc.Grainline(fc.P(ln * 0.1, depth * 0.5), fc.P(ln * 0.9, depth * 0.5)),
        internals=[],
        cut=fc.CutSpec(quantity=2, on_fold=True, fold_edge="cb_fold"),
        label="Collar (cut 2, on fold)",
    )


def build():
    pattern = fc.PatternSet("morning-tailcoat")
    everything = target_piece == "set"
    want = {
        "front": everything or target_piece == "front",
        "back": everything or target_piece == "back",
        "tail": everything or target_piece == "tail",
        "upper_sleeve": everything or target_piece == "upper_sleeve",
        "under_sleeve": everything or target_piece == "under_sleeve",
        "collar": everything or target_piece == "collar",
    }
    if not any(want.values()):
        want = dict.fromkeys(want, True)
    if want["front"]:
        pattern.add(build_front())
    if want["back"]:
        pattern.add(build_back())
    if want["tail"]:
        pattern.add(build_tail())
    if want["upper_sleeve"]:
        pattern.add(build_upper_sleeve())
    if want["under_sleeve"]:
        pattern.add(build_under_sleeve())
    if want["collar"]:
        pattern.add(build_collar())

    if want["front"] and want["back"]:
        pattern.declare_seam(("front", "side"), ("back", "side"), tol=1.5)
        pattern.declare_seam(("front", "shoulder"), ("back", "shoulder"), tol=1.0)
    if want["back"] and want["tail"]:
        # The tail's top is drafted to the MEASURED back waist so the waist seam
        # closes at delta zero.
        pattern.declare_seam(("back", "waist"), ("tail", "waist"), tol=0.8)
    if want["upper_sleeve"] and want["under_sleeve"]:
        pattern.declare_seam(("upper_sleeve", "fore_seam"),
                             ("under_sleeve", "fore_seam"), tol=1.0)
        pattern.declare_seam(("upper_sleeve", "hind_seam"),
                             ("under_sleeve", "hind_seam"), tol=1.0)
    if want["upper_sleeve"] and want["under_sleeve"] and want["front"] and want["back"]:
        us = build_upper_sleeve()
        un = build_under_sleeve()
        cap = us.edge("cap").length(0.05) + un.edge("scye").length(0.05)
        pattern.declare_seam([("upper_sleeve", "cap"), ("under_sleeve", "scye")],
                             [("front", "armscye"), ("back", "armscye")],
                             tol=3.0, ease=cap - ARMSCYE_RUN)

    fabric_width = 1500.0
    total_area = sum(
        p.area() * p.cut.quantity * (2.0 if p.cut.on_fold else 1.0)
        for p in pattern.pieces)
    marker_len = total_area / (fabric_width * 0.62)
    pattern.bom = [
        {"item": "wool herringbone, 320 gsm (morning cloth)",
         "qty": round(marker_len / 10.0) * 10, "unit": "mm_length",
         "note": f"at {fabric_width:.0f} mm width, 62% marker; a grey herringbone "
                 f"is the traditional morning-coat cloth."},
        {"item": "sew-through button", "qty": 5, "unit": "piece",
         "note": f"Yantra4D sew-through-button (notion.hardware_ref) at "
                 f"{button_ligne:.0f} ligne ({BUTTON_MM:.1f} mm): 1 fastening "
                 f"button at the break, 2 waist (tail), 2 cuff. The single button "
                 f"sits at the top of the cutaway sweep."},
        {"item": "hair canvas + shoulder pads", "qty": 1, "unit": "set",
         "note": "a fully canvassed bodice; the tails hang from the waist seam."},
        {"item": "fine edge-stitch thread + needle 90/14", "qty": 1, "unit": "spool",
         "note": f"{EDGESTITCH:.0f} mm edge-stitch on the lapel, the cutaway curve "
                 f"and the tail edges."},
    ]
    pattern.metadata = {
        "fc400_rank": 314,
        "family": "tailoring",
        "tier": 4,
        "fabric_hint": "wool-herringbone",
        "finished_mm": {
            "quarter_chest": round(QUARTER_CHEST, 1),
            "quarter_waist": round(QUARTER_WAIST, 1),
            "bodice_length": round(BODICE_LEN, 1),
            "tail_length": round(tail_length, 1),
            "cutaway_run": round(SWEEP_RUN, 1),
        },
        "solved": {
            "button_y_mm": round(BUTTON_Y, 2),
            "sweep_run_requested_mm": round(_SWEEP_RUN_RAW, 2),
            "sweep_run_floored_mm": round(SWEEP_RUN, 2),
            "sweep_run_was_clamped": bool(abs(SWEEP_RUN - _SWEEP_RUN_RAW) > 0.01),
            "sweep_drop_mm": round(SWEEP_DROP, 2),
            "cutaway_curve_measured_mm": round(CUTAWAY_RUN, 2),
            "lapel_run_measured_mm": round(LAPEL_RUN, 2),
            "armscye_run_measured_mm": round(ARMSCYE_RUN, 2),
            "note": "the cutaway curve is a Bezier whose span (SWEEP_RUN horizontal, "
                    "SWEEP_DROP vertical) is derived from the button break and the "
                    "side, and BOTH are floored — at parameter extremes the derived "
                    "span goes to zero or negative, and a negative span does not "
                    "fail, it inverts the front into geometry the kernel's CCW "
                    "normalization launders into a valid-looking outline. The tails "
                    "are a separate skirt drafted to the measured back waist so the "
                    "waist seam closes.",
        },
        "hardware": "sew-through buttons via Yantra4D (notion.hardware_ref -> "
                    "sew-through-button); the solid's button_ligne is fed from a "
                    "fixed 34-ligne morning button, which also sizes the buttonhole. "
                    "The single fastening button sits at the top of the cutaway.",
    }
    return pattern


result = build()
