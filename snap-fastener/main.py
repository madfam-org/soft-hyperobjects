...
socket_dia  = float(PARAM(lambda: socket_dia, 12.0))
...
socket_dia  = max(6.0, min(socket_dia, 20.0))

MARK_DIA = socket_dia  # the guide-mark circle diameter (mm)
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Snap Fastener Placement Guide",
    )

    pattern = fc.PatternSet("snap-fastener")
    pattern.add(piece)
    pattern.metadata = {
