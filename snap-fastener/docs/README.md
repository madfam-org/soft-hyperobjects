...
[`snap-fit`](https://app.yantra4d.com) cartridge, referenced through
`notion.hardware_ref`.
...
`snap-fit` cartridge and maps the garment `socket_dia` to the solid's `bore_dia`. Resolution is enforced in CI by
...
- **Yantra4D** (`snap-fit`): the fastener as a *solid* — the printable geometry.
