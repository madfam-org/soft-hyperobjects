"""
Crewneck Sweater — FC-100 rank #38. Fashion Cabinet Garment Cartridge.

CUT-AND-SEW knitwear: the sweatshirt architecture (rank #6) redrawn for
sweater knit. Slimmer body (100 mm ease), higher set-in-feel armhole, long
sleeve into a deep cuff rib, and all three ribs — neckband, cuffs, hem band —
derived from measured openings times sweater-rib recovery ratios (sweater
knits stretch more, so the ratios bite harder than fleece). Shoulder seams
carry a "tape shoulder seam" internal marking just inside each shoulder
edge: knits grow, taped shoulders do not.
...
machine-knit (Knitout) version is future work.
...
sweater_ease  = float(PARAM(lambda: sweater_ease, 100.0))   # slimmer than fleece
cuff_ratio    = float(PARAM(lambda: cuff_ratio, 0.72))
hemband_ratio = float(PARAM(lambda: hemband_ratio, 0.88))
neckband_ratio = float(PARAM(lambda: neckband_ratio, 0.80))
cuff_height    = float(PARAM(lambda: cuff_height, 70.0))    # deep sweater cuff
hemband_height = float(PARAM(lambda: hemband_height, 50.0))
neckband_width = float(PARAM(lambda: neckband_width, 30.0))
...
sweater_ease = max(40.0, min(sweater_ease, 300.0))
...
L = body_length
AH = (chest_girth + sweater_ease) / 8.0 + 85.0        # higher armhole: set-in feel
...
def build():
    pattern = fc.PatternSet("crewneck-sweater")
    ...
    if all_pieces or target_piece == "ribs":
        ...
        pattern.add(_rib("neckband", neck_opening * neckband_ratio, neckband_width, 1,
                         "Neckband (rib)"))
    ...
    pattern.metadata = {
        ...
        "knit_note": "cut-and-sew branch; fully-fashioned Knitout version is future work",
        "fabric_gap": "sweater-knit fabric card pending; felpa as stand-in",
        "rib_ratios": {"neck": neckband_ratio, "cuff": cuff_ratio, "hem": hemband_ratio},
        "drafting": "sweatshirt block slimmed for sweater knit; taped shoulders; "
                    "ribs derived from openings",
    }
    return pattern
