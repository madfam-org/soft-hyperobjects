"""
Packable Rain Shell — Fashion Cabinet Garment Cartridge
(FC-400 rank #376, outerwear, Yantra4D-bridged cord-lock).

A featherweight hooded rain shell that stuffs into its own chest pocket: a dropped-shoulder
BODY (front + back), a two-panel HOOD, and a one-piece SLEEVE, all in ripstop, with a drawcord
hem and a drawcord hood face that cinch on cord-locks against the wind. The cord-lock is the
Yantra4D `cord-lock` solid (notion.hardware_ref); its cord channel is bored for the same
cord_dia the shell's casings carry.

Drafting note — the seam that must SOLVE: the hood must set into the MEASURED neckline, not a
neck-girth formula, or it strains the seam in the rain when it is up and loaded. The hood's
neck edge is drafted to the summed measured front + back neck; the hem and hood-face drawcord
casings are sized to the cord doubled plus turn-of-cloth so the cord-lock always fits. The
sleeve cap is bisected against the measured armholes so one cap fits.

Pieces:
  - front  : shell front (cut 2 mirrored); centre-front zip edge.
  - back   : shell back (cut 1 on fold).
  - sleeve : one-piece dropped sleeve (cut 2 mirrored).
  - hood   : one hood half (cut 2, mirrored).
...
cord_dia = float(PARAM(lambda: cord_dia, 4.0))
seam_allowance = float(PARAM(lambda: seam_allowance, 10.0))

# ── Clamps ───────────────────────────────────────────────────────────────────
chest_girth = max(820.0, min(chest_girth, 1520.0))
shell_length = max(560.0, min(shell_length, 920.0))
...
cord_dia = max(2.0, min(cord_dia, 7.0))
seam_allowance = max(6.0, min(seam_allowance, 16.0))

EASE = 240.0                                 # a shell goes over layers
HALF_CHEST = (chest_girth + EASE) / 4.0
HALF_SHOULDER = shoulder_width / 2.0
NECK_W = neck_girth / 6.0 + 10.0
NECK_DROP_F = neck_girth / 6.0 + 20.0
NECK_DROP_B = 26.0
ARMHOLE_DROP = 300.0
SHOULDER_SLOPE = 44.0
CASING_H = max(20.0, cord_dia * 4.0 + 12.0)  # drawcord casing depth
...
def build_front():
    ...
    p_neck_cf = fc.P(0.0, h - NECK_DROP_F + 2.0)
    edges = [
    ...
    internals = [
        fc.Internal("hem-casing",
                    [fc.P(0.0, CASING_H), fc.P(HALF_CHEST, CASING_H)], kind="marking"),
        fc.Internal("stuff-pocket",
                    [fc.P(HALF_CHEST * 0.25, h - NECK_DROP_F - 320.0),
                     fc.P(HALF_CHEST * 0.7, h - NECK_DROP_F - 320.0),
                     fc.P(HALF_CHEST * 0.7, h - NECK_DROP_F - 460.0),
                     fc.P(HALF_CHEST * 0.25, h - NECK_DROP_F - 460.0),
                     fc.P(HALF_CHEST * 0.25, h - NECK_DROP_F - 320.0)], kind="marking"),
    ]
    return fc.Piece(
        "front", edges,
        seam_allowance=seam_allowance,
        allowances={"centre_front": 0.0},
        notches=[fc.Notch("armhole", 0.55, "sleeve cap front match"),
                 fc.Notch("hem", 0.5, "cord exit")],
...
def build_back():
    ...
    return fc.Piece(
        ...
        grainline=None,
        internals=[fc.Internal("hem-casing",
                               [fc.P(0.0, CASING_H), fc.P(HALF_CHEST, CASING_H)],
                               kind="marking")],
...
# ── Hood half, its neck edge = the MEASURED half neckline ────────────────────
_NECK_HALF_RUN = _F.edge("neck").length(0.2) + _B.edge("neck").length(0.2)


def build_hood():
    """One hood half (cut 2, mirrored). Its neck edge = the measured half neckline
    (front neck + back neck); the face edge carries a drawcord casing."""
    neck_run = _NECK_HALF_RUN
    hood_h = max(300.0, neck_run * 1.4)
    hood_w = max(240.0, neck_run * 1.05)
    p_neck_front = fc.P(0.0, 0.0)
    p_neck_back = fc.P(neck_run, 0.0)          # neck edge laid flat along x
    ...
    return fc.Piece(
        ...
        notches=[fc.Notch("neck_edge", 0.5, "shoulder match"),
                 fc.Notch("neck_edge", 0.0, "centre front")],
        grainline=fc.Grainline(fc.P(neck_run * 0.5, 20.0),
                               fc.P(neck_run * 0.5, hood_h - 20.0)),
        internals=[fc.Internal("face-casing",
                               [fc.P(-hood_w * 0.1, hood_h - CASING_H),
                                fc.P(0.0, CASING_H)], kind="marking")],
        cut=fc.CutSpec(quantity=2, mirror=True),
        label="Hood half",
    )
...
def build():
    pattern = fc.PatternSet("packable-rain-shell")
    ...
    if everything:
        ...
        # The hood halves join at centre back, and their neck edges set into the
        # measured neckline (front x2, back x2 = on fold).
    ...
    pattern.bom = [
        {"item": "ripstop nylon (silicone-coated)", "qty": round(marker_len / 10.0) * 10,
         "unit": "mm_length",
         "note": "≈ at 1500 mm width, 76% marker; a silnylon shell packs to a fist and "
                 "sheds a shower — seam-seal the shoulders."},
        {"item": "cord lock", "qty": 2, "unit": "count",
         "note": f"Yantra4D cord-lock (notion.hardware_ref): one at the hem, one at the "
                 f"hood face; the cord channel is bored for {cord_dia:.1f} mm cord."},
        {"item": "drawcord", "qty": round(HALF_CHEST * 4.0 + _NECK_HALF_RUN * 2.0 + 600.0),
         "unit": "mm_length", "note": f"{cord_dia:.1f} mm; threads the hem and hood casings."},
        {"item": "coil zipper (centre front)", "qty": 1, "unit": "count",
         "note": "a lightweight coil zip keeps the shell packable."},
        {"item": "seam tape", "qty": 1, "unit": "roll",
         "note": "tape the shoulder and hood seams or the rain finds them."},
    ]
    pattern.metadata = {
        "fc400_rank": 376,
        "family": "outerwear",
        ...
        "finished_mm": {"chest": round(HALF_CHEST * 4.0, 1),
                        "length": round(shell_length, 1),
                        "cord_dia": round(cord_dia, 1)},
        "solved": {
            "cap_height_mm": round(CAP_H, 2),
            "armhole_total_mm": round(ARMHOLE_F + ARMHOLE_B, 2),
            "neck_half_run_mm": round(_NECK_HALF_RUN, 2),
            "casing_depth_mm": round(CASING_H, 2),
            "note": "the hood's neck edge is drafted to the MEASURED half neckline so it "
                    "sets in without straining the seam under a loaded, up hood; the hem "
                    "and hood-face casings are sized to the cord doubled plus turn-of-"
                    "cloth so the cord-lock always fits; the sleeve cap is bisected "
                    "against the measured armholes.",
        },
        "hardware": "drawcord cord-locks via Yantra4D (notion.hardware_ref -> cord-lock); "
                    "cord_dia = cord_dia. Two locks: hem and hood face.",
    }
    return pattern
