"""
Crewneck Sweatshirt — FC-100 rank #6. Fashion Cabinet Garment Cartridge.

Boxy fleece body, long sleeve with the cap solved to the armholes (same
numeric method as rank #1), and THREE derived rib pieces — neckband, cuffs,
hem band — each computed from a measured opening times a rib recovery ratio.
...
sleeve_length = float(PARAM(lambda: sleeve_length, 560.0))  # apex to cuff seam
fleece_ease   = float(PARAM(lambda: fleece_ease, 140.0))    # boxy total ease
...
cuff_height    = float(PARAM(lambda: cuff_height, 60.0))    # finished rib heights
hemband_height = float(PARAM(lambda: hemband_height, 60.0))
neckband_width = float(PARAM(lambda: neckband_width, 22.0))
...
fleece_ease = max(40.0, min(fleece_ease, 400.0))
...
HPS_Y = L + 20.0
SHOULDER_DROP = 30.0                                  # boxier, flatter shoulder
...
def build_sleeve(cap_target):
    ...
    chw = max(90.0, hb * 0.62)                         # tapers hard into the cuff
    return fc.Piece(
        "sleeve",
        [
...
def build():
    pattern = fc.PatternSet("sweatshirt-crewneck")
    ...
    pattern.metadata = {
        ...
        "drafting": "boxy fleece block; cap solved to armholes; ribs derived from openings",
    }
    return pattern
