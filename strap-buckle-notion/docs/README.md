...
A **Yantra4D-bridged notion** — the 2-D **placement guide** for the side-release or ladder buckle that cinches a strap on a bag, belt, or overall. Fashion
...
[`strap-buckle`](https://app.yantra4d.com) cartridge, referenced through
`notion.hardware_ref`.
...
| `strap_width` | 25.0 | Strap / webbing width (mm) |

## The cross-commons bridge (`notion.hardware_ref`)

This notion's manifest declares a **linked hardware reference** to the Yantra4D
`strap-buckle` cartridge and maps the garment `strap_width` to the solid's `webbing`. Resolution is enforced in CI by
...
- **Yantra4D** (`strap-buckle`): the fastener as a *solid* — the printable geometry.
