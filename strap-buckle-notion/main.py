"""
Strap Buckle — Fashion Cabinet Notion Cartridge (Yantra4D-bridged hard good).

The fastener SOLID is Yantra4D territory (strap-buckle; see the manifest's
...
strap_width = max(10.0, min(strap_width, 50.0))

MARK_DIA = strap_width  # the guide-mark circle diameter (mm)
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Strap Buckle Placement Guide",
    )

    pattern = fc.PatternSet("strap-buckle-notion")
    pattern.add(piece)
    pattern.metadata = {
        ...
        "hardware": "solid geometry delegated to Yantra4D "
                    "(see manifest notion.hardware_ref -> strap-buckle)",
    }
    return pattern
