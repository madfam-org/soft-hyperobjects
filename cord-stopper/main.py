"""
Cord Stopper — Fashion Cabinet Notion Cartridge (Yantra4D-bridged hard good).

The fastener SOLID is Yantra4D territory (cord-lock; see the manifest's
...
MARK_DIA = cord_dia  # the guide-mark circle diameter (mm)
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Cord Stopper Placement Guide",
    )

    pattern = fc.PatternSet("cord-stopper")
    pattern.add(piece)
    pattern.metadata = {
        ...
        "hardware": "solid geometry delegated to Yantra4D "
                    "(see manifest notion.hardware_ref -> cord-lock)",
    }
    return pattern
