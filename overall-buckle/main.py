"""
Overall Buckle — Fashion Cabinet Notion Cartridge (Yantra4D-bridged, Group-B co-creation).

The finding SOLID is Yantra4D territory (overall-buckle; see the manifest's notion.hardware_ref) —
...
strap_width = max(15.0, min(strap_width, 60.0))
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Overall Buckle Placement Guide",
    )
    pattern = fc.PatternSet("overall-buckle")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> overall-buckle)",
    }
    return pattern
