...
A **Yantra4D-bridged notion** — the 2-D **placement guide** for the clip that hangs a pouch or tool from a belt. Fashion
...
[`belt-clip`](https://app.yantra4d.com) cartridge, referenced through
`notion.hardware_ref`.
...
| `belt_width` | 38.0 | Belt width (mm) |

## The cross-commons bridge (`notion.hardware_ref`)

This notion's manifest declares a **linked hardware reference** to the Yantra4D
`belt-clip` cartridge and maps the garment `belt_width` to the solid's `belt_w`. Resolution is enforced in CI by
...
- **Yantra4D** (`belt-clip`): the fastener as a *solid* — the printable geometry.
