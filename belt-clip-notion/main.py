"""
Belt Clip — Fashion Cabinet Notion Cartridge (Yantra4D-bridged hard good).

The fastener SOLID is Yantra4D territory (belt-clip; see the manifest's
...
belt_width  = float(PARAM(lambda: belt_width, 38.0))
...
belt_width  = max(20.0, min(belt_width, 60.0))

MARK_DIA = belt_width  # the guide-mark circle diameter (mm)
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Belt Clip Placement Guide",
    )

    pattern = fc.PatternSet("belt-clip-notion")
    pattern.add(piece)
    pattern.metadata = {
        ...
        "hardware": "solid geometry delegated to Yantra4D "
                    "(see manifest notion.hardware_ref -> belt-clip)",
    }
    return pattern
