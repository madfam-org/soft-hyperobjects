"""
Nehru-collar suit jacket — Fashion Cabinet Garment Cartridge
(FC-500 rank #445, tailoring, T3; y4d shank-button-solid).

A single-breasted suit jacket with a NEHRU COLLAR — a short, standing band collar that meets edge
to edge at the centre front instead of a notched or peaked lapel. The absence of a lapel changes
the front: it runs straight to the neck with no roll line, buttoned to the throat. Hip-length,
set-in sleeve, no lapel.
...
def build_front():
    ...
    return fc.Piece(
        ...
        internals=[fc.Internal("button-stand", [fc.P(0.0, h * 0.2), fc.P(0.0,
                h - NECK_DROP - 20.0)],
                               kind="marking")],
        cut=fc.CutSpec(quantity=2, mirror=True), label="Front (no lapel, straight CF)")
...
def build():
    pattern = fc.PatternSet("nehru-collar-suit")
...
def _finish(pattern):
    ...
    pattern.metadata = {
        "fc500_rank": 445, "family": "tailoring", "fabric_hint": "lana-peinada-traje",
        "silhouette_note": "A single-breasted suit jacket with a short standing Nehru collar and "
            "no lapel; the front runs straight to the throat.",
        ...
        "tailoring": {"cut": "Nehru-collar single-breasted suit jacket, no lapel, set-in sleeve."},
    }
    return pattern
