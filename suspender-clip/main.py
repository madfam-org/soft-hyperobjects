"""
Suspender Clip — Fashion Cabinet Notion Cartridge (Yantra4D-bridged, Group-B co-creation).

The finding SOLID is Yantra4D territory (suspender-clip; see the manifest's notion.hardware_ref) —
...
webbing = float(PARAM(lambda: webbing, 25.0))
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Suspender Clip Placement Guide",
    )
    pattern = fc.PatternSet("suspender-clip")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> suspender-clip)",
    }
    return pattern
