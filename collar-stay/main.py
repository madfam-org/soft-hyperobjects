"""
Collar Stay — Fashion Cabinet Notion Cartridge (Yantra4D-bridged).

The finding SOLID is Yantra4D territory (collar-stay; see the manifest's notion.hardware_ref).
...
stay_len = float(PARAM(lambda: stay_len, 65.0))
base_w = float(PARAM(lambda: base_w, 12.0))
count = int(PARAM(lambda: count, 2))
run_length = float(PARAM(lambda: run_length, 160.0))
end_offset = float(PARAM(lambda: end_offset, 20.0))
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Collar Stay Placement Guide",
    )
    pattern = fc.PatternSet("collar-stay")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> collar-stay)",
    }
    return pattern
