"""
D-Ring / Slider — Fashion Cabinet Notion Cartridge (Yantra4D-bridged, Group-B co-creation).

The finding SOLID is Yantra4D territory (d-ring; see the manifest's notion.hardware_ref) —
...
strap_width = float(PARAM(lambda: strap_width, 25.0))
...
strap_width = max(10.0, min(strap_width, 60.0))
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="D-Ring / Slider Placement Guide",
    )
    pattern = fc.PatternSet("d-ring-slider")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> d-ring)",
    }
    return pattern
