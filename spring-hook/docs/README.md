...
A **Yantra4D-bridged notion** — the 2-D **placement guide** for the spring hook that clips keys, straps, or gear to a webbing tab. Fashion
...
[`carabiner`](https://app.yantra4d.com) cartridge, referenced through
`notion.hardware_ref`.
...
| `tab_width` | 20.0 | Attachment tab width (mm) |

## The cross-commons bridge (`notion.hardware_ref`)

This notion's manifest declares a **linked hardware reference** to the Yantra4D
`carabiner` cartridge and maps the garment `tab_width` to the solid's `inner_w`. Resolution is enforced in CI by
...
- **Yantra4D** (`carabiner`): the fastener as a *solid* — the printable geometry.
