"""
Spring Hook — Fashion Cabinet Notion Cartridge (Yantra4D-bridged hard good).

The fastener SOLID is Yantra4D territory (carabiner; see the manifest's
...
tab_width   = float(PARAM(lambda: tab_width, 20.0))
...
tab_width   = max(10.0, min(tab_width, 40.0))

MARK_DIA = tab_width  # the guide-mark circle diameter (mm)
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Spring Hook Placement Guide",
    )

    pattern = fc.PatternSet("spring-hook")
    pattern.add(piece)
    pattern.metadata = {
        ...
        "hardware": "solid geometry delegated to Yantra4D "
                    "(see manifest notion.hardware_ref -> carabiner)",
    }
    return pattern
