"""
Drawcord Anchor — Fashion Cabinet Notion Cartridge (Yantra4D-bridged).

The finding SOLID is Yantra4D territory (cord-lock; see the manifest's notion.hardware_ref).
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Drawcord Anchor Placement Guide",
    )
    pattern = fc.PatternSet("drawcord-anchor")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> cord-lock)",
    }
    return pattern
