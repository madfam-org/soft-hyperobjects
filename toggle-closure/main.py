"""
Toggle Closure — Fashion Cabinet Notion Cartridge (Yantra4D-bridged).

The finding SOLID is Yantra4D territory (toggle; see the manifest's notion.hardware_ref).
...
barrel_len = float(PARAM(lambda: barrel_len, 38.0))
cord_dia = float(PARAM(lambda: cord_dia, 4.0))
count = int(PARAM(lambda: count, 4))
...
cord_dia = max(1.5, min(cord_dia, 10.0))
count = max(1.0, min(count, 12.0))
run_length = max(40.0, min(run_length, 1000.0))
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Toggle Closure Placement Guide",
    )
    pattern = fc.PatternSet("toggle-closure")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> toggle)",
    }
    return pattern
