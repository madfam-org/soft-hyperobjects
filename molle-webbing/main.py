"""
MOLLE Webbing — Fashion Cabinet Notion Cartridge (Yantra4D-bridged hard good).

The fastener SOLID is Yantra4D territory (molle-clip; see the manifest's
...
column_pitch = float(PARAM(lambda: column_pitch, 38.0))
columns     = int(  PARAM(lambda: columns, 4))
...
column_pitch = max(25.0, min(column_pitch, 50.0))
columns     = max(1.0, min(columns, 12.0))

MARK_DIA = column_pitch  # the guide-mark circle diameter (mm)
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="MOLLE Webbing Placement Guide",
    )

    pattern = fc.PatternSet("molle-webbing")
    pattern.add(piece)
    pattern.metadata = {
        ...
        "hardware": "solid geometry delegated to Yantra4D "
                    "(see manifest notion.hardware_ref -> molle-clip)",
    }
    return pattern
