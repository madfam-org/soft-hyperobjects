...
A **Yantra4D-bridged notion** — the 2-D **placement guide** for the rows of webbing that mount modular pouches to a pack or plate carrier. Fashion
...
[`molle-clip`](https://app.yantra4d.com) cartridge, referenced through
`notion.hardware_ref`.
...
| `column_pitch` | 38.0 | Column pitch (mm, MOLLE ~38) |
| `columns` | 4 | Webbing columns |

## The cross-commons bridge (`notion.hardware_ref`)

This notion's manifest declares a **linked hardware reference** to the Yantra4D
`molle-clip` cartridge and maps the garment `column_pitch` to the solid's `col_pitch`, the garment `columns` to the solid's `panel_cols`. Resolution is enforced in CI by
...
- **Yantra4D** (`molle-clip`): the fastener as a *solid* — the printable geometry.
