"""
T-shirt (V-neck) — FC-100 rank #27. Fashion Cabinet Garment Cartridge.

Rank #1's relaxed drop-shoulder knit block with a TRUE V front neckline: the
front neck is one straight stitch line from the centre front at `v_depth`
below the high point shoulder up to the HPS neck point — a single Line, not a
curve. The neckband is a MITERED V-band: band length derived from the
measured neck opening times a V-appropriate rib ratio (V-bands need more
negative ease than crews), with a centre-V miter notch and a "miter fold"
cross marked at the band centre. Back and solved-cap sleeve are identical to
rank #1 (the cap is bisected until it matches the front + back armholes).
...
v_depth        = float(PARAM(lambda: v_depth, 140.0))        # HPS down to V point at CF
...
neckband_ratio = float(PARAM(lambda: neckband_ratio, 0.82))  # V-band length / opening
...
v_depth = max(60.0, min(v_depth, 250.0, body_length - 150.0))
...
def _body_piece(name, neck_edge, neck_top_y, label, extra_notches=None):
    origin = fc.P(0.0, 0.0)
    edges = [
...
def build_front():
    """Front with a true V: one straight neck edge from the CF V point to HPS.

    Authored CF → HPS so the notch at t=0.0 sits on the V point; the Piece
    constructor's CCW normalization remaps it (t → 1−t) to the same physical
    point when it reverses the ring.
    """
    cf_neck_y = HPS_Y - v_depth
    neck = fc.Edge("neck", [fc.Line(fc.P(0.0, cf_neck_y), fc.P(NW, HPS_Y))])
    return _body_piece(
        "front", neck, cf_neck_y, "Front",
        extra_notches=[fc.Notch("neck", 0.0, "V point")],
    )
...
def build_neckband(front_piece, back_piece):
    """Mitered V-band: length = measured opening × ratio + joins; miter at CF.

    The band centre lands on the V point when sewn; the miter notch and the
    "miter fold" cross mark where the band is folded and stitched into the V.
    The cross is one polyline of axis-aligned retraced strokes (renders as +).
    """
    ...
    return fc.Piece(
        ...
        notches=[fc.Notch("bottom", 0.5, "center V miter")],
        ...
        internals=[
            ...
            fc.Internal(
                "miter fold",
                [fc.P(mid - arm, band_h / 2.0), fc.P(mid + arm, band_h / 2.0),
                 fc.P(mid, band_h / 2.0), fc.P(mid, band_h / 2.0 + arm),
                 fc.P(mid, band_h / 2.0 - arm)],
            ),
        ],
        cut=fc.CutSpec(quantity=1),
        label="Neckband (rib, mitered V)",
    )


def build():
    pattern = fc.PatternSet("t-shirt-v-neck")
    front = build_front()
    back = build_back()
    ...
    pattern.bom = [
        ...
        {"item": "1x1 rib for mitered V neckband", "qty": 1, "unit": "strip",
         "note": "see neckband piece dimensions; miter at the centre notch"},
    ...
    pattern.metadata = {
        ...
        "band_length_mm": round(2.0 * half_opening * neckband_ratio + 2.0 * seam_allowance, 1),
        "armhole_each_mm": round(cap_target / 2.0, 1),
        "fabric_hint": "jersey-algodon",
        "drafting": "rank #1 drop-shoulder knit block; true-V front neck; mitered V-band",
    }
    return pattern
