"""
Magnetic Clasp — Fashion Cabinet Notion Cartridge (Yantra4D-bridged).

The finding SOLID is Yantra4D territory (magnetic-clasp; see the manifest's notion.hardware_ref).
...
disc_dia = float(PARAM(lambda: disc_dia, 18.0))
count = int(PARAM(lambda: count, 1))
run_length = float(PARAM(lambda: run_length, 200.0))
end_offset = float(PARAM(lambda: end_offset, 20.0))

disc_dia = max(10.0, min(disc_dia, 40.0))
count = max(1.0, min(count, 8.0))
run_length = max(40.0, min(run_length, 800.0))
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Magnetic Clasp Placement Guide",
    )
    pattern = fc.PatternSet("magnetic-clasp")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> magnetic-clasp)",
    }
    return pattern
