"""
Jeans Button — Fashion Cabinet Notion Cartridge (Yantra4D-bridged, Group-B co-creation).

The finding SOLID is Yantra4D territory (jeans-button; see the manifest's notion.hardware_ref) —
...
head_dia = float(PARAM(lambda: head_dia, 17.0))
...
count = max(1.0, min(count, 6.0))
run_length = max(40.0, min(run_length, 600.0))
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Jeans Button Placement Guide",
    )
    pattern = fc.PatternSet("jeans-button")
    pattern.add(piece)
    pattern.metadata = {
        "count": count, "run_length_mm": round(run_length, 1),
        "hardware": "finding solid delegated to Yantra4D (notion.hardware_ref -> jeans-button)",
    }
    return pattern
