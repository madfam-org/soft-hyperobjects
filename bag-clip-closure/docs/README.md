...
A **Yantra4D-bridged notion** — the 2-D **placement guide** for the snap clip that seals a roll-top or pouch without a zipper. Fashion
...
[`bag-clip`](https://app.yantra4d.com) cartridge, referenced through
`notion.hardware_ref`.
...
| `clip_width` | 40.0 | Clip width (mm) |

## The cross-commons bridge (`notion.hardware_ref`)

This notion's manifest declares a **linked hardware reference** to the Yantra4D
`bag-clip` cartridge and maps the garment `clip_width` to the solid's `clip_width`. Resolution is enforced in CI by
...
- **Yantra4D** (`bag-clip`): the fastener as a *solid* — the printable geometry.
