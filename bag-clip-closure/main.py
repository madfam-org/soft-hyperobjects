"""
Bag Clip Closure — Fashion Cabinet Notion Cartridge (Yantra4D-bridged hard good).

The fastener SOLID is Yantra4D territory (bag-clip; see the manifest's
...
clip_width  = float(PARAM(lambda: clip_width, 40.0))
...
clip_width  = max(20.0, min(clip_width, 80.0))

MARK_DIA = clip_width  # the guide-mark circle diameter (mm)
...
def build():
    ...
    piece = fc.Piece(
        ...
        label="Bag Clip Closure Placement Guide",
    )

    pattern = fc.PatternSet("bag-clip-closure")
    pattern.add(piece)
    pattern.metadata = {
        ...
        "hardware": "solid geometry delegated to Yantra4D "
                    "(see manifest notion.hardware_ref -> bag-clip)",
    }
    return pattern
